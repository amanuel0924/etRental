...
const HouseDetail = () => {
  ...
  const [sendBrokersRequest, { isLoading: brokerLoaing }] =
    useSendBrokersRequestMutation()
  ...
  const handleBrokerRequest = async (e) => {
    e.preventDefault()
    try {
      console.log({ id, commition: commission })
      await sendBrokersRequest({ id, commition: commission }).unwrap()
    ...
    <div className="text-gray-700 body-font py-5  container mx-auto  bg-white shadow-lg mt-6 rounded-lg h-full w-full flex flex-col items-center ">
      {isLoading ? (
        ...
        <>
          ...
          <div className=" rounded-lg w-full  max-h-fit bg-white   flex flex-wrap  max-w-6xl ">
            <div className="md:w-1/2 w-full ">
              <Feedback refetch={refetch} feedbacks={data?.feedbacks} id={id} />
            </div>
            <div className="md:w-1/2 w-full p-3 overflow-hidden ">
              ...
              {user?.role === "landlord" && (
                <div className="flex flex-col md:flex-row   overflow-x-auto  my-5 p-5 w-full">
                  <div className=" md:flex-1 px-4   my-4 h-fit shadow-lg  ">
                    <h2 className=" m-2 font-bold text-xl  p-3">
                      Broker requests
                    </h2>
